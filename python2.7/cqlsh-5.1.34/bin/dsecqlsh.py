...
# I am fixing that in dse.in.sh, but there may be old stuff floating
...
DSE_CQL_HTML_FALLBACK = 'https://docs.datastax.com/en/cql/3.3/index.html'
...
class DSEShell(OriginalShell):
    def __init__(self, hostname, port, color=False,
        ...
        if use_conn:
            ...
            conn = cqlsh.Cluster(contact_points=(hostname,), port=port, cql_version=cqlver,
            ...
            if debug and ssl:
                sys.stderr.write("Using SSL options: %s\n" % (sslhandling.human_readable_ssl_options(conn.ssl_options),))
        ...
        OriginalShell.__init__(self, hostname, port, color=color, auth_provider=auth_provider,
                               ...
                               tracing_enabled=tracing_enabled, expand_enabled=expand_enabled,
    ...
    def do_execute(self, parsed):
        ...
        if username is None:
            ...
            self.execute_as = {'ProxyExecute': username.encode()}
