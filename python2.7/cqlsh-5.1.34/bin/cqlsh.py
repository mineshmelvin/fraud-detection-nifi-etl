...
DEFAULT_HISTORY_DISABLED = False
...
if os.path.exists(OLD_CONFIG_FILE):
    if os.path.exists(CONFIG_FILE):
        ...
        print('Moving %s to %s.' % (OLD_CONFIG_FILE, CONFIG_FILE))
        os.rename(OLD_CONFIG_FILE, CONFIG_FILE)
OLD_HISTORY = os.path.expanduser(os.path.join('~', '.cqlsh_history'))
if os.path.exists(OLD_HISTORY):
    print('Moving %s to %s.' % (OLD_HISTORY, HISTORY))
...
class Shell(cmd.Cmd):
    ...
    def __init__(self, hostname, port, color=False, auth_provider=None,
        ...
        if use_conn:
            ...
            if self.debug and self.ssl:
                sys.stderr.write("Using SSL options: %s\n" % (sslhandling.human_readable_ssl_options(self.conn.ssl_options),))
    ...
    def show_version(self):
        ...
        vers['dist'] = "DSE" if ((self.conn.protocol_version & 0x40) == 0x40) else "Native"
        vers['protocolVersion'] = self.conn.protocol_version & 0x3f
        vers['version_str'] = 'DSE %s' % vers.get('dse') if vers.get('dse') else 'Cassandra %s' % vers.get('build')
        vers['opt_ssl_version'] = ''
        if self.ssl:
            ssl_version = sslhandling.ssl_version_name(self.conn.ssl_options.get('ssl_version'))
            vers['opt_ssl_version'] = " | %s" % (ssl_version,)
        print("[cqlsh %(shver)s | %(version_str)s | CQL spec %(cql)s | %(dist)s protocol v%(protocolVersion)s%(opt_ssl_version)s]" % vers)
    ...
    def execute_simple_statement(self, statement):
        return self.session.execute_async(statement, trace=self.tracing_enabled)

    def perform_simple_statement(self, statement):
        if not statement:
            return False, None

        future = self.execute_simple_statement(statement)
        result = None
        try:
    ...
    def _columnize_unicode(self, name_list, quote=False):
        ...
        """
        names = [n.encode(self.encoding) for n in name_list]
        if quote:
  ...
  """
    ...
    def do_copy(self, parsed):
        ...
        copyoptnames = [option.lower() for option in parsed.get_binding('optnames', ())]
...
def read_options(cmdlineargs, environment):
    ...
    optvalues.disable_history = option_with_default(configs.getboolean, 'history', 'disabled', DEFAULT_HISTORY_DISABLED)
