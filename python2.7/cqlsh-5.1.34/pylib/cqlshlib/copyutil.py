...
from dse.cqltypes import ReversedType, UserType, BytesType
...
def printerrmsg(shell, msg):
    """
    Because in the tests stderr is printed after stdout, it's convenient to send errors to stdout when
    in DEBUG mode, so that we have access to messages in chronological order.
    """
    printdebugmsg(msg)
    shell.printerr(msg)
...
class SendingChannels(object):
    ...
    def close(self):
        printdebugmsg("Closing sending channels")
...
class ReceivingChannels(object):
    ...
    def close(self):
        printdebugmsg("Closing receiving channels")
        for ch in self.channels:
            try:
                ch.close()
            except Exception:
...
class CopyTask(object):
    ...
    def start_processes(self):
        for i, process in enumerate(self.processes):
            process.start()
            printdebugmsg("Started child process PID {}".format(process.pid))
    ...
    def stop_processes(self):
        for process in self.processes:
            if process.is_alive():
                printdebugmsg("Terminating child process PID {}".format(process.pid))
                process.terminate()
            else:
                printdebugmsg("Child process PID {} already terminated".format(process.pid))

    def send_poison_pill(self, pill):
        """
        Send a poison pill to all child processes, and wait up to 5 seconds to see if they exit cleanly,
        if not they will be terminated during close, see stop_processes()
        """
        printdebugmsg("Parent process sending poison pill to {} child processes".format(self.num_live_processes()))
        for i, child in enumerate(self.processes):
            if child.is_alive():
                self.outmsg.channels[i].send(pill)
        ...
        printdebugmsg("{} child processes still running after {} attempt(s)".format(self.num_live_processes(), 50 - attempts))
    ...
    def validate_columns(self):
        ...
        if not self.columns:
            printerrmsg(shell, "No column specified")
            return False

        for c in self.columns:
            if c not in self.table_meta.columns:
                printerrmsg(shell, 'Invalid column name %s' % (c,))
                return False
    ...
    def __init__(self, fname, shell, columns, options):
        ...
        if self.max_output_size > 0:
            if fname is not None:
                ...
                printerrmsg(shell, "WARNING: maxoutputsize {} ignored when writing to STDOUT".format(self.max_output_size))
...
class ExportTask(CopyTask):
    ...
    def run(self):
        ...
        if self.options.unrecognized:
            printerrmsg(shell, 'Unrecognized COPY TO options: %s' % ', '.join(self.options.unrecognized.keys()))
            return
    ...
    def get_ranges(self):
        ...
        if begin_token and begin_token < min_token:
            printerrmsg(shell, 'Begin token %d must be bigger or equal to min token %d' % (begin_token, min_token))
            return ranges

        if begin_token and end_token and begin_token > end_token:
            printerrmsg(shell, 'Begin token %d must be smaller than end token %d' % (begin_token, end_token))
            return ranges
        ...
        if not ranges:
            printerrmsg(shell, 'Found no ranges to query, check begin and end tokens: %s - %s' % (begin_token, end_token))
    ...
    def export_records(self, ranges):
        ...
        while (failed + succeeded) < total_requests and self.num_live_processes() == num_processes:
            for token_range, result in self.inmsg.recv(timeout=0.1):
                ...
                if token_range is None and result is None:  # a request has finished
                    ...
                    if ranges[token_range]['attempts'] < max_attempts and ranges[token_range]['rows'] == 0:
                        printerrmsg(shell, 'Error for %s: %s (will try again later attempt %d of %d)'
                                    % (token_range, result, ranges[token_range]['attempts'], max_attempts))
                        self.send_work(ranges, [token_range])
                    else:
                        printerrmsg(shell, 'Error for %s: %s (permanently given up after %d rows and %d attempts)'
                                           % (token_range, result, ranges[token_range]['rows'], ranges[token_range]['attempts']))
                        failed += 1
            ...
            if (time.time() - last_recv_time) > child_timeout:
                printerrmsg(self.shell, "No records exported in {} seconds, aborting".format(child_timeout))
                break

        if self.num_live_processes() < len(processes):
            for process in processes:
                if not process.is_alive():
                    printerrmsg(shell, 'Child process %d died with exit code %d' % (process.pid, process.exitcode))

        self.send_poison_pill(pill=(None, None))

        if succeeded < total_requests:
            printerrmsg(shell, 'Exported %d ranges out of %d total ranges, some records might be missing'
...
class ImportErrorHandler(object):
    ...
    def max_exceeded(self):
        if self.insert_errors > self.max_insert_errors >= 0:
            printerrmsg(self.shell, "Exceeded maximum number of insert errors %d" % self.max_insert_errors)
            return True

        if self.parse_errors > self.max_parse_errors >= 0:
            printerrmsg(self.shell, "Exceeded maximum number of parse errors %d" % self.max_parse_errors)
            return True
    ...
    def handle_error(self, err):
        ...
        if err.is_parse_error():
            ...
            printerrmsg(shell, "Failed to import %d rows: %s - %s,  given up without retries"
                        % (len(err.rows), err.name, err.msg))
        else:
            if not err.final:
                printerrmsg(shell, "Failed to import %d rows: %s - %s,  will retry later, attempt %d of %d"
                            % (len(err.rows), err.name, err.msg, err.attempts, self.max_attempts))
            else:
                self.insert_errors += len(err.rows)
                self.add_failed_rows(err.rows)
                printerrmsg(shell, "Failed to import %d rows: %s - %s,  given up after %d attempts"
...
class ImportTask(CopyTask):
    ...
    def validate_columns(self):
        ...
        if not self.valid_columns:
            printerrmsg(shell, "No valid column specified")
            return False

        for c in self.table_meta.primary_key:
            if c.name not in self.valid_columns:
                printerrmsg(shell, "Primary key column '%s' missing or skipped" % (c.name,))
                return False
    ...
    def run(self):
        ...
        if self.options.unrecognized:
            printerrmsg(shell, 'Unrecognized COPY FROM options: %s' % ', '.join(self.options.unrecognized.keys()))
            return
    ...
    def import_records(self):
        ...
        while self.feeding_result is None or self.receive_meter.total_records < self.feeding_result.sent:
            ...
            if self.receive_meter.total_records != last_recv_num_records:
                ...
                printerrmsg(self.shell, "No records inserted in {} seconds, aborting".format(child_timeout))
                break
        ...
        if self.error_handler.num_rows_failed:
            printerrmsg(self.shell, "Failed to process %d rows; failed rows written to %s" %
                        (self.error_handler.num_rows_failed, self.error_handler.err_file))

        if not self.all_processes_running():
            printerrmsg(self.shell, "{} child process(es) died unexpectedly, aborting"
            ...
            self.send_poison_pill(pill=None)
...
class FeedingProcess(mp.Process):
    ...
    def inner_run(self):
        ...
        printdebugmsg("PID {} - Feeding process creating reader".format(self.pid))
        ...
        printdebugmsg("PID {} - Feeding process reader created".format(self.pid))
        channels = self.worker_channels
        max_pending_chunks = self.max_pending_chunks
        sent = 0
        failed_attempts = 0

        printdebugmsg("PID {} - Feeding process entering loop".format(self.pid))
        ...
        printdebugmsg('Feeding process PID {} loop terminated, sending final result to parent process'.format(self.pid))
        self.outmsg.send(FeedingProcessResult(sent, reader))

        # wait for poison pill (None)
        printdebugmsg('Feeding process PID {} waiting for poison pill'.format(self.pid))
        self.inmsg.recv()
...
class ExportProcess(ChildProcess):
    ...
    def inner_run(self):
        ...
        printdebugmsg("PID {} - entering loop".format(self.pid))
        while True:
            if self.num_requests() > self.max_requests:
                if time.time() - last_time > 1:
                    printdebugmsg("PID {} - max num of requests exceeded: {}/{}".format(
                        self.pid, self.num_requests(), self.max_requests))
                    last_time = time.time()
        ...
        printdebugmsg("PID {} - loop terminated".format(self.pid))
    ...
    def maybe_inject_failures(self, partition_key, token_range, attempts):
        ...
        if 'exit_range' in self.test_failures:
            ...
            if start_token >= exit_range['start'] and end_token <= exit_range['end']:
                printdebugmsg('Exiting at range [%d, %d] because of testing...' % (start_token, end_token))
                os._exit(1)  # do not use sys.exit(1), the parent process could take to long to detect it and the test would fail
...
class ImportConversion(object):
    ...
    def get_null_val(self):
        ...
        return ensure_str("0") if self.is_counter else ensure_str("NULL")
    ...
    def get_row_partition_key_values_fcn(self):
        ...
        def serialize_row_multiple(row):
            ...
            for i in partition_key_indexes:
                ...
                length = len(val)
                pk_values.append(struct.pack(ensure_str(">H%dsB" % length), length, val, 0))
    ...
    def maybe_inject_failures(self, batch):
        ...
        if 'exit_batch' in self.test_failures:
            ...
            if exit_batch['id'] == batch['id']:
                printdebugmsg('Child PID {} - Exiting at batch {} because of testing...'.format(self.pid, batch['id']))
                os._exit(1)  # do not use sys.exit(1), the parent process could take to long to detect it and the test would fail
